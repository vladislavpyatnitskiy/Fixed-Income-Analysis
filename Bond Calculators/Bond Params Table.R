...
B.table <- function(P, C, r, y, f = 1, s = 1, VaR = 95, PV = NULL, p = NULL){
  ...
  B <- P*(C*(1/r * (1 - 1 / (1 + r/f)^(y*f))) + 1/(1 + r/f)^(y*f)) # Bond Price
  
  # Duration & Modified Duration #
  P. <- P * (1 + C / f) / (1 + r) ^ (y * f) # Calculate part for principle
  
  for (n in 1:(y * f - 1)){ PV <- cbind(PV, C * P / f / (1 + r) ^ n) # PV
  
    p <- cbind(p, n * PV[n]) } # Coupons
  ...
  l.ratios <- cbind(round(D,3),round(MD,2),round(Cy,2),B*qnorm(1-VaR*.01)*MD*r)
  
  # Set column names 
  colnames(l.ratios) <- c("Duration", "Modified Duration", "Convexity", "VaR")
