...
npv.irr.plt <- function(x, xlim = NULL, h = 0, col, lwd = 3, main = NULL){ 
  ...
  v.plt <- data.frame(as.matrix(seq(0, 1, .01)), v) # Join IRR and NPV
  
  colnames(v.plt) <- c("IRR", "NPV") # Column names 
  
  plot(x=v.plt,type="l",xlim=xlim,main=main,las=1,col=col, lwd = lwd) # Plot 
